...
Стартовый проект намеренно неисправен: успешный запуск до ваших изменений не
ожидается. Это задача на диагностику интеграции. Не переписывайте прикладную
логику и не подменяйте ROS-взаимодействие прямыми вызовами между nodes.
Исправление должно работать после новой сборки, в другом namespace и при
изменённом порядке старта.
...
2. `diagnosis.yaml` в корне `task02/`, заполненный по
   [`docs/diagnosis.template.yaml`](docs/diagnosis.template.yaml).
3. Коммит с результатом работы в ветке `master`.
...
## Внешний контракт одного робота
...
внешним observer и не доверяет изменяемым студентом workflow или локальным
отчётам.
...
Выйдите из shell и остановите окружение:
